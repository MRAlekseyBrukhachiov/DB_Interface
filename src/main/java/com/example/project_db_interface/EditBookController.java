...
public class EditBookController {
    ...
    void save(MouseEvent event) {
        ...
        DataBaseHandler dbHandler = new DataBaseHandler();
        dbHandler.editBook(title, count, weight, language, author_id, genre_id,
                format_id, cover_id, paper_id, bookId);
        clear();
    }
    ...
    void initialize() {
        ...
        try {
            while (resultSet.next()) {  // loop
                String str = String.valueOf(resultSet.getInt("height"))
                        + " / " + String.valueOf(resultSet.getInt("width"));
    ...
    public void setTextField(String title, int count, int weight, String lang, String author,
                             String genre, int format_id, int cover_id, int paper_id, int id) {
        bookId = id;
        addBookAuthorId.setText(author);
        addBookCount.setText(String.valueOf(count));
        coverSelection.getSelectionModel().select(cover_id - 1);
        formatSelection.getSelectionModel().select(format_id - 1);
        addBookGenreId.setText(genre);
        addBookLanguage.setText(lang);
        paperSelection.getSelectionModel().select(paper_id - 1);
        addBookTitle.setText(title);
        addBookWeight.setText(String.valueOf(weight));
    }
}
